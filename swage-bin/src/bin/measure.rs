...
use swage_core::memory::{
    BytePointer, DRAMAddr, Memory, PfnResolver, construct_memory_tuple_timer,
};
use swage_core::util::{MB, ROW_SIZE};
use swage_mmap::{AllocChecker, ConsecCheckPfn};
...
fn mmap(size: usize) -> anyhow::Result<*mut libc::c_void> {
    ...
    unsafe {
        libc::memset(ptr, 0x42, MB(4).bytes());
    }
    Ok(ptr)
}
...
fn alloc_4m_consec() -> anyhow::Result<Memory> {
    ...
    for allocation in allocations.iter_mut() {
        let ptr = mmap(MB(4).bytes())?;
        let mem = Memory::new(ptr as *mut u8, MB(4).bytes());
...
fn alloc_1g_hugepage() -> anyhow::Result<Memory> {
    ...
    Ok(Memory::new(ptr as *mut u8, MB(1024).bytes()))
}
