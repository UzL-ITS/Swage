...
use super::DRAMAddr;
use super::{BytePointer, MemoryTupleTimer, pfn_offset::CachedPfnOffset};
...
/// Resolves the PFN-to-virtual-address offset in rows.
///
/// Used by allocators to determine DRAM row alignment for allocated memory.
pub trait PfnOffsetResolver {
    /// Determines the PFN offset by testing memory access timing.
    ///
    /// # Arguments
    ///
    /// * `mem_config` - DRAM configuration
    /// * `conflict_threshold` - Timing threshold for bank conflicts (CPU cycles)
    /// * `timer` - Memory timing measurement interface
    /// * `progress` - Optional progress reporting
    ///
    /// # Returns
    ///
    /// The PFN offset in rows, or None if detection fails
    fn pfn_offset(
        &self,
