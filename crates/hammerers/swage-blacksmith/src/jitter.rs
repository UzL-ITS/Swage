use core::slice;
use iced_x86::{
    BlockEncoderOptions, Decoder, DecoderOptions, Formatter, Instruction, NasmFormatter,
    code_asm::*,
};
use log::debug;
...
impl FromStr for FencingStrategy {
    type Err = UnknownStrategyError;
    fn from_str(s: &str) -> std::result::Result<FencingStrategy, Self::Err> {
        match s {
...
impl Program {
    ...
    pub(crate) unsafe fn call(&self) -> u64 {
        let jit_function_ptr = unsafe { self.code.as_ptr().offset(self.start as isize) };
    ...
    pub(crate) fn write(&self, filename: &str) -> Result<()> {
